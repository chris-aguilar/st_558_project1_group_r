...
plot.county <- function(df, var_name = "value", state_val = "NC", group = "top", row_display = 5) {

# generate plot variable
  county_variable_subset <-  df |>
...
if(group == "top") {
  ...
  g_county + 
    ...
    ylab("County") +
    labs(title = paste("Average", var_name, "by county"), subtitle = str_to_sentence(paste(group, row_display, "counties")))

}

plot.county(census_data$county_data, state_val = "VA", var_name = "enrollment", group = "top", row_display = 10)
...
ASSIGNMENT SPECIFICATIONS GO BELOW.

We separate each prompt and code with headers.
