...
# What "putting it all together" should be?

Going back through the lectures, professor Post mentioned that functions should be self-contained, which is a similiar concept in other computer programming courses like SAS. 

With this in mind, we need/should combine the separate functions and their supporting code associated with preparing the data into one "parent" function. 

My attempt at this using the EDU01a URL is below. Currently getting the error:  

> ℹ Use `spec()` to retrieve the full column specification for this data.
ℹ Specify the column types or set `show_col_types = FALSE` to quiet this message.
Error in census_data[county_idx, ] : incorrect number of dimensions
...
# step 1 and 2  
  ...
  df <- read_csv(url) # main df for use throughout data manipulation

  read_and_pivot <- function(df, default_var_name) {
    df |> 
      select(Area_name, STCOU, ends_with("D")) |> # step 1
      rename(area_name = Area_name) |> 
      pivot_longer(ends_with("D"), values_to = default_var_name) # step 2
...
# step 3. Extract the survey year. 


  extract_survey_and_year <- function(census_data) {
    res <- census_data |>
    mutate(
        measurement = substr(name, 1, 7),
        # year = paste0("19", substr(name, 8, 9)) |> as.integer())
...
# step 4. Create state indices for splitting the data between county and non-county data frames. 
...
# putting the helper functions together to execute 
    res <- read_and_pivot(url, default_var_name) |> # step 1, 2 
      extract_survey_and_year() |> # step 3
      split_census_data() # step 4, 5, 6
...
# EDU01a_df <- prepare_census_data("https://www4.stat.ncsu.edu/~online/datasets/EDU01a.csv", "enrollment")
