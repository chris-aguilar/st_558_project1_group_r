...
# Calling on a dataframe of clas "state" with a variable-of-interest called "enrollment"
plot.state(census_data$state_data, "enrollment")
...
plot.county <- function(df, state_var = "NC", group = "top", row_display = 5) {

# generate plot variable
  county_variable_subset <-  census_data[[1]] |>

# filter the data to show only the state specified
    filter(state == "NC") |>
    
# find the overall mean of the statistic for each area name
    group_by(area_name) |> 
    summarize(area_name, "mean_enrollment" = mean(enrollment)) |>
    distinct(area_name, mean_enrollment) |>
    arrange(desc(mean_enrollment))
  
# sort the values depending on the selection of top or bottom
    #ifelse(get(group) == "top", 
    #       arrange(desc(mean_enrollment)), 
...
# obtain the top or bottom x number of area_names where the number is given by the user or default and filter the data to only include the top or bottom area names for graphing
    Display_num_subset <- head(county_variable_subset, 5)

  g_county <- ggplot(Display_num_subset, aes(area_name, mean_enrollment )) 
  g_county + geom_point() +
    labs(x = "Area Name", y = "TBD") +
...
Testing above code 
```{r}
# specify data frame, state, top/bottom, and number of results displayed in the function

plot.county <- function(df, state_var = "NC", group_var = "top", row_display = 5) {

# generate plot variable
  county_subset <- df[[1]] |>  

# filter the data to show only the state specified
  filter(state == get(state_var)) |>  
...
# sort the values depending on the selection of 'top' or 'bottom'
  dplyr::ifelse(get(group) == "top", 
       arrange(desc(mean_enrollment)),
       arrange(mean_enrollment) 
      )

# obtain the top or bottom x number of area_names where the number is given by the user or default and filter the data to only include the top or bottom area names for graphing
  plot_num_subset <- head(county_subset, get(row_display))

  g_county <- ggplot(plot_num_subset, aes(area_name, mean_enrollment )) 
  g_county + geom_point() + 
    labs(x = "Area Name", y = "TBD") + 
...
#plot.county(census_data) 
#commented out to allow for render, need to troubleshoot errors

# Error comment below:

#Error in ifelse(distinct(summarize(group_by(filter(df[[1]], state == get(state_var)), : unused argument (arrange(mean_enrollment))
