...
extract_survey_and_year <- function(df) {
  ...
  mutate(
      ...
      year = ifelse(
        substr(name, 8, 8) %in% c("4", "5", "6", "7", "8", "9"), paste0("19", substr(name, 8, 9)) |> as.integer(),
        ifelse(
...
For the State class plotting method we've developed a function that plots the mean value of the variable of interest, in this case `enrollment`, across the years for each Division. The twenty "Error" values for Division have been removed in generating the subset prior to plotting the data.
...
Next we created a similar plotting function with more flexiblity. The plotting funciton allows the user to specify a state of interest and determine how many of the top or bottom counties in the state they want to observe the average enrollment for.
...
# obtain the top or bottom x number of area_names where the number is given by the user or default and filter the data to only include the top or bottom area names for graphing
  ...
  # sorting plot order depending on ascending or descending
  # Sorting data 
if(group == "top") {
  g_county <- Display_num_subset |> ggplot(aes(y = forcats::fct_reorder(area_name, mean_value), x = mean_value))
} else if(group == "bottom") {
  g_county <- Display_num_subset |> ggplot(aes(y = forcats::fct_reorder(area_name, desc(mean_value)), x = mean_value))
} else {
  stop("group must be in one of c('top', 'bottom')")
}
  g_county + 
    geom_col() +
    xlab(paste("Average", var_name)) +
    ylab("County") +
    labs(title = paste("Average", var_name, "by county in", state_val), subtitle = str_to_sentence(paste(group, row_display, "counties")))
...
We can now take the above methods `plot.state` and `plot.county` to create time series plots for our numeric values of interest, and plots of the average value of interest by county.

# Putting it together

## Data processing

Run the data processing function on the two enrollment URLs, specifying an appropriate name for the enrollment data column.
...
Run the data combining function to put the two data frames into one object with two data frames.
...
Use the plot function on the 'county' data frame to produce the following plots.
...
Runing the data processing function on the below URLs provided us with data frames `PST01a` through `PST01d` that we will combine into one object with separate data frames for state data and county data. 
...
Running our data combining function three times created the object `PST01_combined` that will be used to generate our state and county plot graphics for analysis.  
```{r}
...
Use the function `plot.state` to generate the plot of `PST01_combined`'s state data frame.
```{r}
...
Use the function `plot.county` to generate the plot of `PST01_combined`'s county data frame using the below specified criteria. 
...
Using the default value of "enrollment" for the variable "population" renders an error message. 

```{r}
# plot.county(PST01_combined)

# commenting out the plot to allow the rest of the program to work, permitting rendering, committing, and pushing to GitHub.


```

> Error in `summarize()`:
> ℹ In argument: `mean_value = mean(get(var_name))`.
> ℹ In group 1: `area_name = "Alamance, NC"`.
> Caused by error in `get()`:
> ! object 'enrollment' not found
> Backtrace:
>   1. global plot.county(PST01_combined)
>  12. base::get(var_name)
> Error in summarize(group_by(filter(df$county_data, state == state_val),  : 
>  
> ℹ In group 1: `area_name = "Alamance, NC"`.
> Caused by error in `get()`:
> ! object 'enrollment' not found
